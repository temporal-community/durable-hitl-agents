...
**"LangGraph calls itself a runtime — so why Temporal on top?" (the scope answer)**
> "LangChain positions LangGraph as a *runtime* — durable execution, HITL, persistence — a peer to Temporal, Inngest, DBOS. But this demo uses LangGraph as a *framework*: just its graph/loop abstraction for the Dispatch agent's reasoning loop. The checkpointer here is `InMemorySaver` — scratch state, not real durability. The kill-tested durability is **Temporal's event history**. And here's the scope answer: LangGraph's durability is *within its own framework and threads* — it doesn't reach across to ADK. Temporal's durable execution **spans both frameworks**. That cross-framework boundary is exactly what the Cross-Framework tab proves, and it's why a durable-execution runtime sits beneath the framework rather than competing with its loop."
